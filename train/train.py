...
def prepair_datasets(df, df_test, n_classes, level,
    ...
    Returns:
        Кортеж из обучающего, валидационного и тестового датасетов, токенизатора,
        функции для создания батчей и весов классов
    """
    # Загрузка токенизатора
    tokenizer = AutoTokenizer.from_pretrained(pre_trained_model_name, do_lower_case=True)

    # Подготовка разделения данных - убрали преобразование в категории для списков
    ...
    # Стратифицированное разделение - используем строковое представление для стратификации
    ...
    weights = torch.tensor(len(train_df) / (class_counts * n_classes), dtype=torch.float32)
    ...
    datasets = {}
    for name, data in [("train", train_df), ("valid", valid_df), ("test", df_test)]:
...
def prepair_model(n_classes,
    ...
    config = LoraConfig(
        ...
        modules_to_save=['classifier', 'bert.pooler']  # Модули, которые нужно сохранить полностью
    )
...
def prepair_compute_metrics(n_classes):
    ...
    def compute_metrics(pred):
        ...
        preds = torch.sigmoid(torch.tensor(pred.predictions).float())  # Применяем сигмоиду для получения вероятностей
...
class CustomTrainer(Trainer):
    ...
    Расширяет стандартный Trainer из transformers для поддержки взвешенных потерь
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        """
        labels = inputs.pop("labels").to(self.args.device).float()  # Преобразуем метки в float
...
def save_parameters(dir_name, 
    ...
    Args:
        ...
        minimal_number_of_elements_RGNTI2: Минимальное количество элементов RGNTI2
