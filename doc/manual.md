...
Команда **server** запускает локальный сервер на заданом порту и IP-адресе.
### Аргументы команды server
|**Аргумент**|**Ключ**|**Допустимые значения**|**Обязательный аргумент**|**По-умолчанию**|
|:-------------|:-------------|:-------------|:-------------|:-------------|
|Порт сервера|-p, --port|Целое число|нет|8000|
|Адрес хоста|-a, --address|IP-адрес или имя хоста|нет|localhost|
|Выбор устройства выполнения|-dv, --device|**cpu**, **cuda:0**|нет|cpu|
|Включение режима без логов|-s|-|нет|-|
...
```
После запуска сервер становится доступен по адресу `http://<адрес>:<порт>`, например:  
`http://localhost:8080`.